...
- 新增独立模型配置模块：增加 `model_providers` 表、独立管理接口和“模型配置”页面，支持 provider 基础信息、远端候选模型、enabled models 配置和手动添加模型能力。优化模型配置弹窗布局：弹窗固定高度仅展示已启用模型（过多时区域内部滚动、操作按钮固定），点击「获取远程模型」后从弹窗内底部滑出候选面板展示远端模型并支持面板内搜索/过滤与滚动，避免候选列表过长撑高弹窗导致整页滚动。
