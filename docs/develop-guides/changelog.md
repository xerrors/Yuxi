...
- 新增同步子智能体向父智能体提问机制：通过 `task` 启动的 `SubAgentBackend` 会获得仅运行时可见的 `ask_for_main_agent` 工具，缺少父上下文或需要决策时以 LangGraph interrupt 暂停 child run；`task` 将问题、run ID 和 thread ID 作为结构化 `input_required` 结果交回父智能体，父智能体可直接调用 `answer_subagent_question`，或先用 `ask_user_question` 向用户补充信息，再通过 child checkpoint resume 继续同一子任务。child resume 会保留模型快照、父线程文件、Skills 和权限作用域，并按用户、父 conversation、child relation 与最新待回答 run 校验归属；相同回答重试复用已有 resume run，过期问题拒绝重复恢复。该机制首版仅注入同步 `task`，后台 `subagent_start` 的主动提问与父 Agent 自动唤醒仍留待后续实现。
