...
每条更新说明不宜太长，不宜过于详细，应当以简明扼要的方式描述新增、修复或破坏性变更的内容，分清主次。

## v0.7.2 (current)

- 修复知识图谱构建并发仅在固定 20 条批次内瞬时生效的问题：移除前端写死的 `batch_size=20`，后端改为数据库游标生产者、固定数量 LLM 抽取 worker 与独立 structure writer 的持续队列；任一抽取完成后会立即补充下一条，Neo4j/PostgreSQL 结构写入不再占用抽取并发槽。进一步将耗时的 embedding/Milvus 从单 chunk writer 解耦：entity/triple 在 PostgreSQL 中维护可恢复的向量状态，独立 worker 按唯一 ID 批量 embedding 并使用 Milvus upsert 幂等写入，失败自动重试三次；chunk 仅在结构和全部引用向量完成后标记为已构建，新增 reconcile 接口恢复失败/过期索引或只重建图谱向量。LLM 抽取同样支持单 chunk 最多三次尝试，并通过 `graph_extraction_details` 持久化状态、次数和最后错误；局部抽取失败不再把正常结束的批量任务标记为失败，前端可点击“抽取失败”随机查看最多 10 个样例 Chunk，下次索引会自动再次处理这些 Chunk。已有 `extraction_result` 的 chunk 不重复调用 LLM，已有已索引数据升级时不会重新 embedding；任务日志和前端状态分别展示抽取、结构、向量进度及失败数量，成功重试后不再被旧 failed/cancelled 任务持续标记为失败。
- 丰富模型选型参考信息：接入 `@opencode-ai/models` 内置 snapshot，补充模型上下文、能力和价格等信息；模型选择器移除价格悬浮提示并关闭搜索自动完成，底部增加“配置模型”入口并跳转到模型供应商管理；模型供应商候选列表支持美元与人民币价格切换，默认美元，人民币按固定汇率 `1 USD = ¥7` 换算，并明确 models.dev 数据与固定汇率仅供参考；候选模型工具栏统一靠右排列搜索、币种和类型筛选控件，类型文案改为“对话 / 向量 / 重排”。DashScope 中国站内置标识修正为 `alibaba-cn`，`alibaba` 改为使用 `dashscope-intl.aliyuncs.com` 的国际站定义。
