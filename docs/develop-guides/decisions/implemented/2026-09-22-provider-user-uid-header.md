...
| provider 创建、编辑接口可读写 `include_user_uid`，默认关闭且只接受 JSON 布尔值 | 字符串或数字被 HTTP schema 强制转换为 true，绕过 service 校验 | `providers/service.py`、`model_provider_router.py` | `pytest test/unit/server/test_model_provider_router.py test/unit/services/test_model_provider_service.py` | 请求模型拒绝 `"true"`/`"false"`/`1`/`0`；service 拒绝 `"true"`/`1`/`None`/`[]` | Passed |
