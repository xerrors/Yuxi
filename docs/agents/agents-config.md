...
`tools`、`knowledges` 和 `skills` 未配置或为 `null` 时，运行时使用当前用户可访问的全部资源；显式保存空列表表示不启用该类资源。`mcps` 未配置、为 `null` 或空列表时不直接添加 MCP 服务器；显式填写列表后，运行时只使用其中仍可用的服务器。编辑 MCP 字段时，未选中服务器不会直接加入智能体。已激活 Skill 声明的 MCP 依赖仍会按需加载。`preload_skills` 未配置、为 `null` 或空列表时不预加载；预加载候选项限于当前 Agent 已启用的 Skill。
...
编辑页只提交修改过的配置字段。修改可见的资源选择时，后端保留当前管理员不可访问的既有引用，并拒绝新增无权访问的引用；保留的引用维持原相对顺序，新选择追加到末尾。界面显示当前不在可选范围内的既有选择数量。选择“清空全部”会显式移除全部已选引用，包括不可访问项；子智能体对应的操作显示为“使用全部”，恢复为全部可访问的子智能体。通过 API 更新 `config_json.context` 时，省略字段保留原值；工具、知识库和 Skill 的 `null` 表示使用全部，空列表表示不启用；MCP 的 `null` 或空列表表示不直接添加服务器，预加载 Skill 的 `null` 或空列表表示不预加载；非空列表修改可见部分并保留不可见的既有选择。
