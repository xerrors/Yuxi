...
minio_image=$(docker compose config --format json | jq -r '.services.minio.image')
test -n "$api_image" && test -n "$provisioner_image" && test -n "$minio_image"
...
build_with_cache docker/minio/Dockerfile "$minio_image" ./docker/minio "${3:-}"
