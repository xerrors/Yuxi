...
Yuxi 是一个面向 RAG、知识图谱和多智能体工作流的知识库平台。用户在 Vue 前端中配置智能体、知识库、工具、Skills、MCP 与 SubAgents；前端通过 `/api` 调用 FastAPI；后端服务层协调数据库、对象存储、向量库、图数据库、LangGraph 运行态和沙盒；长耗时智能体运行交给主 worker 与独立的 subagent worker 异步执行，并通过事件流回到前端。
...
- `worker-dev`：ARQ 主运行 worker，处理普通聊天与恢复 run。
- `subagent-worker-dev`：ARQ 子运行 worker，通过独立队列执行 subagent run，避免父 run 等待子 run 时耗尽同一执行池。
...
5. `worker-dev` 执行普通 LangGraph run；subagent run 进入独立队列并由 `subagent-worker-dev` 执行。两类 worker 复用同一运行函数，中间件按上下文挂载知识库工具、Skills、MCP、附件与沙盒能力。
...
- 普通 run 与 subagent run 必须使用不同的 ARQ 队列和 Worker 容量；不能让同步等待子 run 的父任务与其子任务重新共用同一个有界执行池。
