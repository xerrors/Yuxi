...
`/` 是公开首页；登录后的核心工作区是 `/agent`。`/extensions` 对所有登录用户开放，其中 Skills 对普通用户可见，知识库对管理员及具备个人知识管理能力的辅导人员开放，工具和 MCP 管理能力仅管理员可见；Dashboard 仅超级管理员可访问。个人知识库复用创建者及空共享范围，仅所有者可访问。后端权限检查始终是最终边界，前端守卫只负责页面体验。
