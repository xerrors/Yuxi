...
def test_office_file_entry_uses_pdf_as_default_preview(tmp_path) -> None:
    kb = make_kb(tmp_path)

    entry = kb._knowledge_file_entry("db1", "file1", kb.files_meta["file1"])

    assert entry["preview_modes"] == ["original", "pdf", "parsed"]
    assert entry["default_preview_mode"] == "pdf"
...
async def test_read_office_pdf_preview_converts_and_caches_pdf(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("yuxi.services.file_preview.convert_office_to_pdf", fake_convert)

    response = await kb.read_file_preview("db1", "file1", "pdf")
    download = await kb.get_file_download("db1", "file1", "pdf")

    assert response["preview_type"] == "pdf"
    assert response["supported"] is True
    assert minio_client.objects[("knowledgebases", "db1/preview/file1.pdf")] == b"%PDF-1.4\nconverted"
    assert download["filename"] == "demo.pdf"
